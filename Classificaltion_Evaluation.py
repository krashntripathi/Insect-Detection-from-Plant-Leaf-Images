...
def PT(fpr, sensitivity):
    ...
    pt = (math.sqrt((sensitivity / 100) * (fpr / 100)) - (fpr / 100)) / ((sensitivity / 100) - (fpr / 100))
...
def FM(sensitivity, precision):
    ...
    fm = math.sqrt((precision / 100) * (sensitivity / 100))
    return fm * 100
...
def net_evaluation(actual, predict):
    if len(actual.shape) == 3 and len(predict.shape) == 3:
        actual = cv.cvtColor(actual, cv.COLOR_RGB2GRAY)
        predict = cv.cvtColor(predict, cv.COLOR_RGB2GRAY)
    if actual.max() > 1.0:
        actual = actual / 255.0
        actual = (actual >= 0.5).astype(np.uint8)
    if predict.max() > 1.0:
    ...
    f1score = F1SCORE(TP, FP, FN)
    mcc = MCC(TP, TN, FP, FN)
    EVAL = [TP, TN, FP, FN, Dice, IoU, accuracy, psnr, mse, sensitivity, specificity, precision, fpr, fnr, npv,
            fdr, f1score, mcc]
    return EVAL
